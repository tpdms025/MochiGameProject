...
public class EnhancementCell : UIReuseItemCell
{
    ...
    [SerializeField] private TextMeshProUGUI nextAmountText;
    ...
    private void ChangeUI(ProductCellData item)
    {
        ...
        if (item.nextLevel == item.level)
        {
            ...
            amountText.text = string.Format("{0}%", CurrencyParser.ToCurrencyString(item.currentAmount));
        }
        else
        {
            ...
            nextAmountText.text = string.Format("{0}%", CurrencyParser.ToCurrencyString(item.nextAmount));
    ...
    private void ToggleUI_NextInfo(bool turnOn)
    {
        ...
        nextAmountText.gameObject.SetActive(turnOn);
